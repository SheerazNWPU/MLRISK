...
def get_mul_probability_input_X(id2contvalue, class_num, pair_ids, interval_boundary_points):
    ...
    #print("pair_ids")
    #print(pair_ids)
    #print("Interval Boundary Points")
    #print(interval_boundary_points)
    ...
    for k in range(len(pair_ids)):
        ...
        #print(class_num)
        #print(continue_values)
        #print(continue_values[0])
        for i in range(class_num):
            #print(continue_values[0][i])
...
#def calculate_rules_feature_mu_sigma(ob_ids, matrix, class_num, ob_labels):
#    
#    _labels = []
#    i = 0
#    for _id in ob_ids:
...
def calculate_rules_feature_mu_sigma(ob_ids, matrix, class_num, ob_labels):
    ...
    for _id in ob_ids:
        for label in range(class_num):
            if (matrix[i][label] == 0):
                continue
            true_label_str = ob_labels.get(_id)
            true_label = [float(x) for x in true_label_str[1:-1].split()]
    ...
    mu = np.average(labels)
    delta = (labels - mu) ** 2
    sum_delta = np.sum(delta)
    sigma = sum_delta / np.maximum(len(_labels) - 1, 1)
    #print([mu, sigma])
    return [mu, sigma]